...
`Tier 9` を手動クリアすると `Insight` が保留されます。Prestigeを実行すると通常進行はリセットされますが、Insightと恒久アップグレードは残ります。

初回Prestige後は `Tier EX` と `Challenge` が解放されます。Challengeは制限つきの周回で、`Tier 9` を手動クリアすると専用通貨 `Seal` を獲得できます。

Sealアップグレードでは、自動ソルバー解放に必要な手動クリア回数の短縮、Insightを獲得できる最低Tierの引き下げ、補助/自動クリアによるInsight獲得などを解放できます。
...
- サーバー、アカウント、オンラインランキング、クラウドセーブ、PWA、音声、エディタ、URL 共有、外部テレメトリはまだありません。
